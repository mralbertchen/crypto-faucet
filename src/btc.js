...
const CHAIN_RPC_URL =
  ...
  "http://username:password@bitcoin-cbe1628199c1624f.elb.us-east-1.amazonaws.com:8332";
const startIndex = 7;
...
module.exports.sendTx = async (amount, destination) => {
  ...
  const payload = {
    ...
    id: Date.now(),
    method: "listunspent",
    params: [0, 9999999, [faucetBTCAddress]]
  };
  ...
  if (!result.length) {
    ...
    console.log(
      `Faucet address not imported in BTC node, reimporting. (This could take a few minutes...)`
    );
    await nodeClient.callRPC({
      payload: {
        ...
        params: [faucetBTCAddress, "TestFaucet", true]
      }
    });
